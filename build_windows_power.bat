...
if exist "%ROOT_DIR%dist\PowerS3Browser" (
  powershell -NoProfile -Command "Compress-Archive -Path '%ROOT_DIR%dist\PowerS3Browser\*' -DestinationPath '%ROOT_DIR%dist\PowerS3Browser-windows.zip' -Force" || exit /b 1
)

echo Build complete: %ROOT_DIR%dist\PowerS3Browser\PowerS3Browser.exe
if exist "%ROOT_DIR%dist\PowerS3Browser-windows.zip" echo Packaged zip: %ROOT_DIR%dist\PowerS3Browser-windows.zip
